...
void loop() {
  ...
  //turbo logo
  //get boost value
  int oilpsi = getOilPSI();  //1 atmosphere is roughtly 14.6 psi
  if (oilpsi > peakPSI){
    peakPSI = oilpsi;
  }
  String SpeakPSI = String(peakPSI);
  String Soilpsi = String(oilpsi);
...
int getBoost() {
  float rPSI = analogRead(36);
  //float ResultPSI = (rPSI*(.00488)/(.022)+20)/6.89 - atmo; //leave of the /6.89 for kpa
  float ResultPSI = (((rPSI / 4095) + 0.04) / 0.004) * 0.145 - atmo; //by 0.145 to calc psi //4096 values on esp32
  /*rPSI = rPSI + 1 ;
    if (rPSI >= 20) {
    rPSI = 0;
    }*/
...
int getOilPSI() {
  ...
  if (psival < 257) {
    return (0); //110 swings higher than the gauge goes which will alert the user to something being wrong
  }
